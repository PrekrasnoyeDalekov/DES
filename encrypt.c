...
uint32_t feistel(uint32_t R, uint64_t Ki){
    ...
    extendedR ^= Ki;
    uint32_t S_out = Subsitute(extendedR);
    uint32_t P_out = 0;
    for(int i = 0;i < 32;i++){
        bit = (S_out >> (32 - P[i])) & 0x1;
        P_out |= (bit << (31 - i));
    }
    return P_out;
}
...
uint32_t Subsitute(uint64_t extendedR){
    uint32_t S_out = 0;
    for(int i = 0;i < 8;i++){
        uint8_t row = ((extendedR >> (46 - i*6)) & 0x2) | ((extendedR >> (47 - 6*i)) & 0x1);
        uint8_t col = (extendedR >> (43 - i*6)) & 0xF;
        uint64_t S_out_i = S[i][row*16 + col];
        S_out |= (S_out_i << (28 - i*4));
    }
    return S_out;
}
